...
impl AppConfig {
    fn load_from_env() -> Result<Self, Box<dyn Error>> {
        dotenv::dotenv().ok();
        let sip_host = env::var("SIP_SIGNALING_SERVICE_LISTEN_ADDRESS")
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    info!(
        ...
        "RabbitMQ exchange'i deklare edildi."
    );
        ...
        match delivery {
            Ok(delivery) => {
                ...
                match serde_json::from_slice::<TerminationRequest>(&delivery.data) {
                    ...
                    Err(e) => {
                        error!(error = %e, "Geçersiz sonlandırma isteği formatı.");
                    }
                }
            }
            Err(e) => {
                error!(error = %e, "RabbitMQ'dan mesaj alınırken hata oluştu.");
            }
        }
    }
}
...
async fn handle_invite(
    ...
    let dialplan_result =
        match create_secure_grpc_channel(&config.dialplan_service_url, "dialplan-service").await {
            Ok(channel) => DialplanServiceClient::new(channel)
                .resolve_dialplan(dialplan_req)
                .await,
            Err(e) => Err(Status::unavailable(format!(
                "Dialplan servisine bağlanılamadı: {}",
                e
            ))),
        };
...
async fn cleanup_old_transactions(transactions: ActiveCalls) {
